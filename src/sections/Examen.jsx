...
export function Examen({ surTermine, surQuitter }) {
  ...
  const tempsEcoule = useCallback(() => {
    if (reponseChoisie === null && !montrerExplication) {
      ...
      nouveauxResultats[serieActuelle] = [
        ...nouveauxResultats[serieActuelle],
        {
  ...
  function questionSuivante() {
    if (questionActuelle < QUESTIONS_PAR_SERIE - 1) {
      ...
      // Examen terminé
      const totalBonnes = resultatsSeries
        .flat()
        .filter((r) => r.correct).length;
      ajouterXP(totalBonnes * 15 + 500);
      if (totalBonnes >= 48) setNouveauBadge(BADGES_DISPONIBLES.EXAMEN_REUSSI.nom);
    ...
    <div className="examen-page">
      <div className="examen-contenu">
        ...
        <div className="examen-entete">
          <button onClick={surQuitter} className="examen-bouton-quitter">
            Quitter
          </button>
        ...
        <div className="examen-progression">
          <div className="examen-progression-labels">
            <span>
              Série {serieActuelle + 1} — Question {questionActuelle + 1}/
              {QUESTIONS_PAR_SERIE}
            </span>
            <span>
              {Math.round(((questionActuelle + 1) / QUESTIONS_PAR_SERIE) * 100)}
              %
            </span>
          </div>
          <div className="examen-progression-piste">
            <div
              ...
              style={{
                width: `${((questionActuelle + 1) / QUESTIONS_PAR_SERIE) * 100}%`,
              }}
            />
          </div>
        </div>

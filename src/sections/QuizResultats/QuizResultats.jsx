    ...
    <div className="quiz-resultats-page">
      <div className="quiz-resultats-contenu">
        <div className="quiz-resultats-carte">
          ...
          {/* Icône résultat */}
          <div
            className={`quiz-resultats-icone-cercle ${reussi ? "quiz-resultats-icone-cercle--succes" : "quiz-resultats-icone-cercle--echec"}`}
          >
          ...
          {reussi ? (
            <button
              onClick={surTermine}
              className="quiz-resultats-bouton-succes"
            >
              <CheckCircle />
              Continuer
            </button>
            ...
            <>
              <button
                onClick={surTermine}
                className="quiz-resultats-bouton-echec"
              >
                Retour à l'accueil
              </button>
              <p className="quiz-resultats-note">
          ...
          <div className="quiz-revision-liste">
            {resultats.map((resultat, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`quiz-revision-item ${resultat.correct ? "quiz-revision-item--correct" : "quiz-revision-item--incorrect"}`}
                >
                  <div className="quiz-revision-item-corps">
                    {resultat.correct ? <CheckCircle /> : <XCircle />}
                    <div>
                      <p className="quiz-revision-question">{q.question}</p>
                      <p className="quiz-revision-reponse">
                        Votre réponse :{" "}
                        {resultat.reponseChoisie >= 0
                          ? q.options[resultat.reponseChoisie]
                          : "Temps écoulé"}
                      </p>
                      {!resultat.correct && (
                        <p className="quiz-revision-bonne">
                          Bonne réponse : {q.options[q.correctAnswer]}
                        </p>
                      )}
                      <p className="quiz-revision-explication">
                        {q.explanation}
                      </p>
                    </div>
                  </div>
